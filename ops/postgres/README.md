...
database URLs. PostgreSQL remains private. `homelab-postgres` is canonical;
...
3. Run `import_argus_legacy.sh` without `--apply`, review the report, take a
   ...
   down migration.
4. Schedule `backup_shared_postgres.sh` on the homelab. Its destination must be
   outside the live PostgreSQL data directory and covered by separate storage
   protection. Retention is the union of 7 daily, 5 weekly, and 12 monthly sets.
5. Run `verify_restore.sh` with operator-named `argus_restore_*` and
   `atlas_restore_*` targets. It verifies the backup manifest, refuses
   production database names, and always drops both validated scratch targets
   on exit.
6. Mount the evidence JSON read-only into Argus and configure
