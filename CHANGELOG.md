...
- **Accepted-operation HTTP authority and additive v2 contract** — legacy HTTP
  ...
  retrieval sessions, and fail-closed activation.
- **Pre-execution HTTP transport guard** — exact Host and Origin policy,
  ambiguous-credential rejection, bounded JSON bodies, narrow CORS, and remote
  production startup validation run before provider or persistence work.
...
- **Per-caller provider tier caps** — `ARGUS_CALLER_TIER_CAPS=hermes*:1,mac-agents:1,maya:1` enforces, server-side, that matching callers never route to providers above the given tier (e.g. tier-3 one-time-credit providers). Enforced in the broker regardless of client flags.
